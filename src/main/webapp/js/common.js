...
async function readInputFile(ths, event) {
    const file = event.target.files[0];
    if (file.type.match("image/*")) {
        if (file.size > 1000000) {
            standBy("이미지를 불러오는 중입니다.", "잠시만 기다려주세요.")
        }
        // 압축 옵션 설정
        const options = {
            maxSizeMB: 2,
        ...
        if (compressedFileSizeMB > options.maxSizeMB) {
            errorShow("[400]", "이미지 파일은 최대 2MB까지 업로드 가능합니다.");
            return;
        }
        const newFileList = new DataTransfer();
        newFileList.items.add(new File([compressedFile], compressedFile.name));

        event.target.files = newFileList.files;
        const imgElements = $(ths).siblings("img");
        imgElements[0].src = URL.createObjectURL(compressedFile);
        commonHide();
    } else {
        errorShow("[400]", "이미지(jpg, jpeg, png) 형식의 파일만 업로드 가능합니다.");
    }
};

function uploadImg(){
    const formData = new FormData();
    formData.append('thumb', $("#upload_input")[0].files[0]);
    $.ajax({
            url: "/upload/img",
            method: "post",
            data: formData,
